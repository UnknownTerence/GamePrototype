...
@export var InventorySlotPrefab : PackedScene = preload("res://inventory_slot.tscn")
...
func getInventoryData() -> Array[InventorySlot]:
	return InventorySlots
# selection: 0 shop, 1 campfire
func updateInventoryData(data : ItemData, slotID : int, selection : int):
	if selection == 0:
		InventorySlots[slotID].FillSlot(data)
	elif selection == 1:
		InventorySlots[slotID].FillSlot(area.ItemTypes[3])
